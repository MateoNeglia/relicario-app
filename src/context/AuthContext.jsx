...
export const AuthProvider = ({ children }) => {
    ...
    const interceptor = axios.interceptors.response.use(
      ...
      async (error) => {
        ...
        if (
          ...
          try {
            const res = await axios.post(`${config.BACKEND_URL}/auth/refresh`, {
  ...
  const login = async (identifier, password) => {
    try {
      const res = await axios.post(`${config.BACKEND_URL}/auth/login`, { identifier, password });
      const { accessToken, refreshToken } = res.data;
      const decoded = isTokenValid(accessToken);
      if (!decoded) {
  ...
  const register = async (username, email, password, role = 'user', isAdmCreated = false) => {
    try {
      const res = await axios.post(`${config.BACKEND_URL}/auth/register`, {
        username,
        email,
        password,
        role,
  ...
  const updateUser = async (formData) => {
    try {
      ...
      const res = await axios.patch(`${config.BACKEND_URL}/auth/profile`, formData, {
        headers: {
